...
class Settings(BaseSettings):
        ...
        if USE_NGROK == False:
                ...
                if not WEBHOOK_URL:
                        logging.error("Error: ngrok disabled, but no self URL provided. Missing config item MERAKI_TARGET_WEBHOOK_URL")
                        sys.exit(1)
        else:
        ...
        if not WEBHOOK_NAME: WEBHOOK_NAME = 'api-generated_Discord'
        if USE_NGROK == None: USE_NGROK = True      
...
meraki = MerakiWebhook(settings.MERAKI_API_KEY,
